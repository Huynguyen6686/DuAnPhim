...
@RestController
@RequestMapping("/api/admin/loai-ghe")
@CrossOrigin("*")
public class LoaiGheController {

    @Autowired
    private LoaiGheService service;

    @GetMapping
    public ResponseEntity<ApiResponse<List<LoaiGheResponse>>> getAll() {
        return ResponseEntity.ok(ApiResponse.success(service.getAll(), "Lấy danh sách thành công"));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<LoaiGheResponse>> getById(@PathVariable("id") UUID id) {
        return ResponseEntity.ok(ApiResponse.success(service.getById(id), "Lấy thông tin thành công"));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<LoaiGheResponse>> create(@Valid @RequestBody LoaiGheRequest request) {
        return ResponseEntity.ok(ApiResponse.success(service.create(request), "Thêm mới thành công"));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<LoaiGheResponse>> update(@PathVariable("id") UUID id, @Valid @RequestBody LoaiGheRequest request) {
