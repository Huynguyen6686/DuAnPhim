...
@RestController
@RequestMapping("/api/admin/phan-loai-do-tuoi")
@CrossOrigin("*")
public class PhanLoaiDoTuoiController {

    @Autowired
    private PhanLoaiDoTuoiService service;

    @GetMapping
    public ResponseEntity<ApiResponse<List<PhanLoaiDoTuoiResponse>>> getAll() {
        return ResponseEntity.ok(ApiResponse.success(service.getAll(), "Lấy danh sách thành công"));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<PhanLoaiDoTuoiResponse>> getById(@PathVariable("id") UUID id) {
        return ResponseEntity.ok(ApiResponse.success(service.getById(id), "Lấy thông tin thành công"));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<PhanLoaiDoTuoiResponse>> create(@Valid @RequestBody PhanLoaiDoTuoiRequest request) {
        return ResponseEntity.ok(ApiResponse.success(service.create(request), "Thêm mới thành công"));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<PhanLoaiDoTuoiResponse>> update(@PathVariable("id") UUID id, @Valid @RequestBody PhanLoaiDoTuoiRequest request) {
